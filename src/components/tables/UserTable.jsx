    ...
    const columns = useMemo(
        () => [
                    ...
                    <div className="flex items-center gap-3">
                        {row.original.profile_picture ? (
                            <img
                                ...
                                className="w-12 h-12 rounded-full aspect-square object-cover border-2 border-slate-200"
                            />
                        ) : (
            ...
            },
            {
                ...
                header: "Role",
                cell: ({ row }) => {
                    const role = row.original.role; // raw role
                    const displayRole = row.original.displayRole; // pretty label
                    const roleColors = {
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="table-count">
                            Total: {filteredData?.length || 0} users
                        </p>
                    </div>
                    {rightAction}
                </div>
            ...
            <div className="px-6 py-4 border-t border-slate-200/50 bg-slate-50/30">
                <div className="flex items-center justify-between">
                    <span className="text-sm text-slate-500">
                        ...
                        {table.getPageCount()} ({filteredData?.length || 0}{" "}
                        total)
                    </span>
                    <div className="flex gap-2">
                        <button

// components/modals/FormModal.jsx - UPDATED
...
const FormModal = ({
  ...
  const handleTooltipMouseLeave = () => setActiveTooltip(null);
  ...
  return (
    <>
      ...
      {activeTooltip && (
        <div
          className="fixed bg-slate-800 text-white text-xs rounded-md py-2 px-3
        ...
        <div className="flex min-h-full items-center justify-center p-4 sm:p-6">
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[95vh] overflow-hidden">
            ...
            <form onSubmit={onSubmit}>
              {/* Content */}
              <div className="p-5 max-h-[calc(95vh-180px)] overflow-y-auto space-y-5">
                {/* Message */}
                {message && (
                  <div
                ...
                {fields.length > 0 &&
                  fields.map(f => (
                    <div key={f.name} className="input-container">
                      <label className="input-label-modern">
                        ...
                        {f.withTooltip && (
                          <Info
                            ...
                            onMouseEnter={e =>
                              handleTooltipMouseEnter(f.tooltipText, e)
                            }
                      ...
                      {f.error && <p className="error-message">{f.error}</p>}
                    </div>
                  ))}

                {/* Children for step components */}
                {children}
              ...
              <div className="flex gap-3 pt-2 px-5 pb-5 border-t">
                {footer ? (
                  footer
                ) : (
                  <>

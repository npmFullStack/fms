...
import { XMarkIcon, InformationCircleIcon, CubeIcon } from "@heroicons/react/24/outline";
  ...
  const onFormSubmit = async (data) => {
    try {
      ...
      if (result.success) {
        ...
        setErrorMessage(result.error || "Failed to update ship. Please try again.");
      }
    } catch (error) {
      console.error("Error submitting form:", error);
      setErrorMessage(error.message || "Failed to update ship. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-100 max-h-[95vh] overflow-hidden">
          ...
          <div className="relative bg-gradient-to-r from-emerald-600 to-emerald-700 rounded-t-2xl px-6 py-3 text-center">
            ...
            <div className="flex flex-col items-center">
              ...
              <h2 className="text-xl font-bold text-white">Update Ship</h2>
            </div>
          </div>

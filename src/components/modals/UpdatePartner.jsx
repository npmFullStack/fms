...
const UpdatePartner = ({ isOpen, onClose, partner }) => {
  ...
  useEffect(() => {
    if (partner) {
      ...
      setValue("contactEmail", partner.contact_email || "");
      setValue("contactPhone", partner.contact_phone || "");
      setValue("website", partner.website || "");
      
      if (partner.type === 'trucking') {
        setValue("serviceRoutes", partner.service_routes || []);
      }
  ...
  const onSubmit = async data => {
    try {
      ...
      if (partner.type === 'trucking' && data.serviceRoutes) {
      ...
      const result = await updatePartner(partner.id, formData, partner.type);

      if (result.success) {
        setSuccessMessage(`${partner.type === 'shipping' ? 'Shipping line' : 'Trucking company'} updated successfully`);
        setTimeout(() => {
          handleClose();
        }, 1500);
      } else {
        setErrorMessage(result.error || `Failed to update ${partner.type === 'shipping' ? 'shipping line' : 'trucking company'}. Please try again.`);
      }
    } catch (error) {
      console.error("Error submitting form:", error);
      setErrorMessage(error.message || `Failed to update ${partner.type === 'shipping' ? 'shipping line' : 'trucking company'}. Please try again.`);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className={`relative bg-gradient-to-r ${partner.type === 'shipping' ? 'from-blue-600 to-blue-700' : 'from-orange-600 to-orange-700'} rounded-t-2xl px-6 py-3 text-center`}>
            ...
            <div className="flex flex-col items-center">
              ...
              <h2 className="text-xl font-bold text-white">
                Update {partner.type === 'shipping' ? 'Shipping Line' : 'Trucking Company'}
              </h2>
            </div>
          </div>
            ...
            <div className="space-y-5">
              ...
              <div className="input-container">
                ...
                <input
                  ...
                  type="text"
                  {...register("name")}
                  placeholder={`Enter ${partner.type === 'shipping' ? 'shipping line' : 'trucking company'} name`}

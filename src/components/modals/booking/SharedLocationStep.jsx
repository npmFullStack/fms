// components/modals/booking/SharedLocationStep.jsx
...
const SharedLocationStep = ({
  ...
  // Watch current location values
  const selectedProvince = useWatch({ control, name: `${locationType}_province` });
  const selectedCity = useWatch({ control, name: `${locationType}_city` });
  const selectedBarangay = useWatch({ control, name: `${locationType}_barangay` });
  const selectedStreet = useWatch({ control, name: `${locationType}_street` });

  // Watch pickup values (for delivery step)
  ...
  useEffect(() => {
    if (selectedBarangay) {
      geocode(`${selectedBarangay}, ${selectedCity}, ${selectedProvince}, Philippines`, 1).then(results => {
        if (results[0]) {
          setLocationCoords({
            lat: +results[0].lat,
            lng: +results[0].lon
  ...
  // --- Geocode pickup location (for delivery step) ---
  useEffect(() => {
    if (!isDeliveryStep) return;
    
    if (pickupProvince && pickupCity && pickupBarangay && pickupStreet) {
    ...
    } else if (pickupProvince && pickupCity && pickupBarangay) {
    ...
    } else if (pickupProvince && pickupCity) {
      const cityPickupAddress = `${pickupCity}, ${pickupProvince}, Philippines`;
      geocode(cityPickupAddress, 1).then(results => {
        if (results[0]) {
          setPickupCoords({
            lat: parseFloat(results[0].lat),
            lng: parseFloat(results[0].lon)
    ...
    } else if (pickupProvince) {
      const provincePickupAddress = `${pickupProvince}, Philippines`;
      geocode(provincePickupAddress, 1).then(results => {
        if (results[0]) {
          setPickupCoords({
            lat: parseFloat(results[0].lat),
            lng: parseFloat(results[0].lon)
  ...
  }, [isDeliveryStep, pickupStreet, pickupBarangay, pickupCity, pickupProvince]);

  // --- Marker positions ---
  const positions = useMemo(() => {
    const pts = [];
    if (pickupCoords && isDeliveryStep) pts.push([pickupCoords.lat, pickupCoords.lng]);
    if (locationCoords) pts.push([locationCoords.lat, locationCoords.lng]);
    ...
    return pts;
  }, [pickupCoords, locationCoords, originPortObj, destinationPortObj, isDeliveryStep]);

  const initialCenter = (() => {
    if (positions.length > 0) return positions[0];
    if (locationCoords) return [locationCoords.lat, locationCoords.lng];
    if (pickupCoords && isDeliveryStep) return [pickupCoords.lat, pickupCoords.lng];
    return [12.8797, 121.774];
  })();

  const initialZoom = positions.length > 0 ? 10 : (locationCoords || pickupCoords) ? 10 : 6;
  ...
  const markerColor = isDeliveryStep ? "red" : "green";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <Select
            ...
            onChange={o => {
              setValue(`${locationType}_province`, o ? o.value : "");
              setValue(`${locationType}_city`, "");
              setValue(`${locationType}_barangay`, "");
              setValue(`${locationType}_street`, "");
              setStreetInputValue("");
              setStreetOptions([]);
            }}
            ...
            styles={selectStyles}
          />
          {errors[`${locationType}_province`] && (
            <p className="error-message">{errors[`${locationType}_province`].message}</p>
          )}
        </div>
        ...
        <div>
          ...
          <Select
            ...
            onChange={o => {
              setValue(`${locationType}_city`, o ? o.value : "");
              setValue(`${locationType}_barangay`, "");
              setValue(`${locationType}_street`, "");
              setStreetInputValue("");
              setStreetOptions([]);
            ...
            styles={selectStyles}
          />
          {errors[`${locationType}_city`] && (
            <p className="error-message">{errors[`${locationType}_city`].message}</p>
          )}
        </div>
        ...
        <div>
          ...
          <Select
            ...
            onChange={o => {
              setValue(`${locationType}_barangay`, o ? o.value : "");
              setValue(`${locationType}_street`, "");
              setStreetInputValue("");
              setStreetOptions([]);
            }}
            ...
            styles={selectStyles}
          />
          {errors[`${locationType}_barangay`] && (
            <p className="error-message">{errors[`${locationType}_barangay`].message}</p>
          )}
        </div>
        ...
        <div>
          ...
          <Select
            ...
            onChange={o => {
              if (o) {
                ...
                setValue(`${locationType}_street`, val);
                setStreetInputValue(val);
                if (o.lat && o.lon) {
                  setLocationCoords({ lat: o.lat, lng: o.lon });
                }
              } else {
                setValue(`${locationType}_street`, "");
                setStreetInputValue("");
            ...
            onInputChange={(inputValue, actionMeta) => {
              if (actionMeta.action === "input-change") {
                ...
                setValue(`${locationType}_street`, inputValue);
              }
            }}
            ...
            isDisabled={!selectedCity}
            styles={selectStyles}
            placeholder={!selectedCity ? "Select city first" : "Type to search…"}
          ...
          />
          {errors[`${locationType}_street`] && (
            <p className="error-message">{errors[`${locationType}_street`].message}</p>
          )}
        </div>
      </div>
      ...
      <div className="bg-gray-50 p-2 rounded-lg border">
        <h4 className="text-xs font-medium mb-1 text-center">Map Legend</h4>
        <div className="flex justify-center items-center gap-3 text-xs flex-wrap">
          ...
          <div className="flex items-center gap-1">
            <div className={`w-3 h-3 ${isDeliveryStep ? 'bg-red-500' : 'bg-green-500'} rounded-full`}></div>
            <span>{isDeliveryStep ? 'Delivery' : 'Pickup'}</span>
          </div>
          <div className="flex items-center gap-1">
          ...
          <div className="flex items-center gap-1">
            <div className={`w-4 h-0.5 ${isDeliveryStep ? 'bg-green-500' : 'bg-green-500'}`}></div>
          ...
          <div className="flex items-center gap-1">
            <div
              ...
              style={{
                backgroundImage: "repeating-linear-gradient(to right, #3b82f6, #3b82f6 2px, transparent 2px, transparent 4px)"
              }}
            ></div>
            <span>Sea Route</span>
          </div>
      ...
      <div className="h-96 w-full rounded-lg overflow-hidden border relative z-0">
        ...
        >
          ...
          {/* Pickup marker (green) - only show on delivery step */}
          {isDeliveryStep && pickupCoords && (
            <Marker
          ...
          {locationCoords && (
            ...
            >
              <Popup>
                <div className="text-sm">
                  <strong>{isDeliveryStep ? 'Delivery' : 'Pickup'} Location:</strong>
                  <br />
          ...
          {!isDeliveryStep && locationCoords && originPortObj && (
            <Polyline
              positions={[
          ...
          {/* Pickup to Origin Port route (green) - for delivery step */}
          {isDeliveryStep && pickupCoords && originPortObj && (
            <Polyline
              positions={[
          ...
          {/* Destination Port to Delivery route (red) - only on delivery step */}
          {isDeliveryStep && destinationPortObj && locationCoords && (
            <Polyline
              positions={[
                [destinationPortObj.lat, destinationPortObj.lng],
                [locationCoords.lat, locationCoords.lng]
...
export default SharedLocationStep;
...
const SharedLocationStep = ({
    ...
    // --- Geocode pickup location (for delivery step) - IMPROVED VERSION ---
    useEffect(() => {
        if (!isDeliveryStep) {
        ...
        // Only proceed if we have pickup data
        if (!pickupProvince) {
            console.log("No pickup province available"); // Debug log
            setPickupCoords(null);
            return;
        }
        ...
        // If street is very long (like yours), try simpler versions first
        if (pickupStreet && pickupStreet.length > 50) {
            // Try just the first part of the street address
            const shortStreet = pickupStreet.split(",")[0].trim();
            if (pickupBarangay && pickupCity && pickupProvince) {
                addressVariations.push(
                    `${shortStreet}, ${pickupBarangay}, ${pickupCity}, ${pickupProvince}, Philippines`
                );
            }
            if (pickupCity && pickupProvince) {
                addressVariations.push(
                    `${shortStreet}, ${pickupCity}, ${pickupProvince}, Philippines`
                );
            }
        }
        ...
        if (pickupStreet && pickupBarangay && pickupCity && pickupProvince) {
            addressVariations.push(
                `${pickupStreet}, ${pickupBarangay}, ${pickupCity}, ${pickupProvince}, Philippines`
            );
        }
        if (pickupBarangay && pickupCity && pickupProvince) {
            addressVariations.push(
                `${pickupBarangay}, ${pickupCity}, ${pickupProvince}, Philippines`
            );
        }
        if (pickupCity && pickupProvince) {
            addressVariations.push(
                `${pickupCity}, ${pickupProvince}, Philippines`
            );
        }
        if (pickupProvince) {
            addressVariations.push(`${pickupProvince}, Philippines`);
        }

        console.log("Trying address variations:", addressVariations); // Debug log

        // Try geocoding each variation until one succeeds
        const tryGeocode = async () => {
            for (let i = 0; i < addressVariations.length; i++) {
                const address = addressVariations[i];
                console.log(`Trying geocoding attempt ${i + 1}:`, address);

                try {
                    const results = await geocode(address, 1);
                    console.log(`Geocoding attempt ${i + 1} results:`, results);
                    ...
                    if (results && results.length > 0 && results[0]) {
                        ...
                        console.log(
                            "✅ Successfully set pickup coords:",
                            coords
                        );
                        setPickupCoords(coords);
                        return; // Success, stop trying other variations
                    }
                } catch (error) {
    ...
    const positions = useMemo(() => {
        ...
        if (pickupCoords && isDeliveryStep) {
        ...
        if (locationCoords) {
            pts.push([locationCoords.lat, locationCoords.lng]);
            console.log("Added location position:", [
                locationCoords.lat,
                locationCoords.lng
            ]); // Debug log
        }
        ...
        console.log("Final positions array:", pts); // Debug log
        return pts;
    }, [
        pickupCoords,
        locationCoords,
    ...
    const initialZoom =
        positions.length > 0 ? 10 : locationCoords || pickupCoords ? 10 : 6;
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    {errors[`${locationType}_province`] && (
                        <p className="error-message">
                            {errors[`${locationType}_province`].message}
                        </p>
                    )}
                </div>
                ...
                <div>
                    ...
                    {errors[`${locationType}_city`] && (
                        <p className="error-message">
                            {errors[`${locationType}_city`].message}
                        </p>
                    )}
                </div>
                ...
                <div>
                    ...
                    {errors[`${locationType}_barangay`] && (
                        <p className="error-message">
                            {errors[`${locationType}_barangay`].message}
                        </p>
                    )}
                </div>
                ...
                <div>
                    ...
                    {errors[`${locationType}_street`] && (
                        <p className="error-message">
                            {errors[`${locationType}_street`].message}
                        </p>
                    )}
                </div>
            </div>
            ...
            <div className="bg-gray-50 p-2 rounded-lg border">
                ...
                <div className="flex justify-center items-center gap-3 text-xs flex-wrap">
                    ...
                    <div className="flex items-center gap-1">
                        <div
                            className={`w-3 h-3 ${
                                isDeliveryStep ? "bg-red-500" : "bg-green-500"
                            } rounded-full`}
                        ></div>
                    ...
                    <div className="flex items-center gap-1">
                        <div
                            className={`w-4 h-0.5 ${
                                isDeliveryStep ? "bg-green-500" : "bg-green-500"
                            }`}
                        ></div>
                    ...
                    {locationCoords && (
                        ...
                        >
                            <Popup>
                                <div className="text-sm">
                                    <strong>
                                        {isDeliveryStep ? "Delivery" : "Pickup"}{" "}
                                        Location:
                                    </strong>
                                    <br />

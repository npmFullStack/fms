...
const BookingStep5 = ({ control, partners = [] }) => {
  ...
  useEffect(() => {
    const fetchContainerDetails = async () => {
      if (data.container_ids && data.container_ids.length > 0 && data.shipping_line_id) {
        try {
          const response = await api.get(`/bookings/available-containers/${data.shipping_line_id}`);
  ...
  return (
    <div className="p-3 bg-gray-50 rounded-lg">
      ...
      <div className="bg-white p-3 rounded border mt-3">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          <div>
            <span className="text-gray-600 text-xs">Company/Consignee:</span>
            <p className="font-medium truncate">{data.consignee || "—"}</p>
          </div>
          <div>
            <span className="text-gray-600 text-xs">Contact Name:</span>
            <p className="font-medium truncate">{data.consignee_name || "—"}</p>
          </div>
          <div className="col-span-2">
            <span className="text-gray-600 text-xs">Phone:</span>
            <p className="font-medium truncate">{data.consignee_phone || "—"}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white p-3 rounded border mt-3">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          <div>
            ...
            <p className="font-medium text-blue-600 text-base">{formatVolumeInfo()}</p>
          </div>
          <div>
          ...
          <div className="col-span-2">
            <span className="text-gray-600 text-xs">Containers:</span>
            <p className="font-medium text-sm">{formatContainerList()}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="font-medium truncate">{data.quantity || 1}</p>
          </div>
          <div>

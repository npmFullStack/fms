...
const BookingStep4DL = ({ control, errors, setValue }) => {
  ...
  // Pickup (watched from PL step, but we show it here too)
  ...
  useEffect(() => {
    const fetchPickupCoords = async () => {
      if (pickupStreet && pickupBarangay && pickupCity && pickupProvince) {
        const fullAddress = `${pickupStreet}, ${pickupBarangay}, ${pickupCity}, ${pickupProvince}, Philippines`;
        const results = await geocode(fullAddress);
        if (results && results.length > 0) {
          const first = results[0];
          setPickupCoords({
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <Select
            value={
              provinceOptions.find(opt => opt.value === selectedProvince) || null
            }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <Select
            value={
              barangayOptions.find(opt => opt.value === selectedBarangay) ||
              null
            }
        ...
        >
          ...
          {pickupCoords && (
            <Marker position={pickupCoords} icon={markerIcon("green")}>
              <Popup>
                Pickup Location: {pickupStreet}, {pickupBarangay}, {pickupCity},{" "}
                {pickupProvince}
              </Popup>
            </Marker>
          )}

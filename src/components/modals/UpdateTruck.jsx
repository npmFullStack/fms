...
import { XMarkIcon, InformationCircleIcon, TruckIcon } from "@heroicons/react/24/outline";
import { useEffect } from "react";

const UpdateTruck = ({ isOpen, onClose, onSubmit, truck, truckingCompanyId }) => {
  const {
    message,
    isLoading,
    setIsLoading,
  ...
  useEffect(() => {
    if (truck) {
      ...
      setValue("plateNumber", truck.plate_number || "");
      setValue("remarks", truck.remarks || "");
      setValue("truckingCompanyId", truckingCompanyId);
    }
  }, [truck, truckingCompanyId, setValue]);

  const onFormSubmit = async (data) => {
    try {
      setIsLoading(true);
      setSuccessMessage("Updating truck...");

      const result = await onSubmit(truck.id, data);
      ...
      if (result.success) {
        ...
        setErrorMessage(result.error || "Failed to update truck. Please try again.");
      }
    } catch (error) {
      console.error("Error submitting form:", error);
      setErrorMessage(error.message || "Failed to update truck. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-100 max-h-[95vh] overflow-hidden">
          ...
          <div className="relative bg-gradient-to-r from-emerald-600 to-emerald-700 rounded-t-2xl px-6 py-3 text-center">
            ...
            <div className="flex flex-col items-center">
              ...
              <h2 className="text-xl font-bold text-white">Update Truck</h2>
            </div>
          </div>
            ...
            <div className="space-y-5">
              ...
              <div className="input-container">
                ...
                <input
                  ...
                  className={`input-field-modern ${errors.name ? "input-error" : ""}`}
                />
              ...
              <div className="input-container">
                <label htmlFor="plateNumber" className="input-label-modern">
                  Plate Number <span className="text-slate-400 text-sm ml-1">(Optional)</span>
                </label>
                <input
                  id="plateNumber"
                  type="text"
                  ...
                  className={`input-field-modern ${errors.plateNumber ? "input-error" : ""}`}
                />
                {errors.plateNumber && (
              ...
              <div className="input-container">
                <label htmlFor="remarks" className="input-label-modern">
                  Remarks <span className="text-slate-400 text-sm ml-1">(Optional)</span>
                </label>
                <textarea
                  id="remarks"
                  {...register("remarks")}

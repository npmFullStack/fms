...
const AddBooking = ({ isOpen, onClose }) => {
  ...
  // Check if current booking mode is port-to-port
  ...
  const handleNext = async () => {
    ...
    // Handle step navigation based on booking mode
    if (currentStep === 2 && isPortToPort) {
      // Skip steps 3, 4, and 5 and go directly to step 6 (BookingStep5 - Review)
      setCurrentStep(6);
  ...
  const handlePrev = () => {
    // Handle previous navigation based on booking mode
    if (currentStep === 6 && isPortToPort) {
      // If we're on review step and port-to-port, go back to step 2
      setCurrentStep(2);

...
const AccountsReceivable = () => {
    ...
    const arRecordsForTable = useMemo(() => {
        if (safeARRecords.length > 0) {
            return safeARRecords.map(ar => ({
                ...
                total_expense: ar.total_expense || 0,
            ...
            return safeBookings.map(booking => {
                ...
                const volume = booking.quantity > 1 ? `${booking.quantity}x${containerSize}` : containerSize;
    ...
    // Calculate statistics - FIXED the amount collected display
    const stats = useMemo(() => {
        const totalReceivable = arRecordsForTable.reduce((sum, r) => sum + (r.total_amount || r.pesos || 0), 0);
        ...
        const overdueCount = arRecordsForTable.filter(r => 
            r.payment_status === "OVERDUE" || (r.aging > 30 && (r.balance > 0 || r.amount_paid === 0))
        ).length;
